...
enum entryEvent {keyCardUnlock, pinEntryUnlock, rfidScanUnlock, failedPin, failedrfidScan, powerUp};
...
#define displayTimeoutTime 5000
...
void failureTone(){
 //Todo, need to find or make proper tone for failure
}
...
void checkForDisplayBacklight(){
  long int brightness = constrain(map(displayOffTime - millis() + displayTimeoutTime / 2, 0, displayTimeoutTime / 2, 0, 255), 0, 255);
  analogWrite(DISPLAY_BACKLIGHT_PIN, brightness);
}
...
void turnOnBacklight(){
  turnOnBacklight(displayTimeoutTime);
}
