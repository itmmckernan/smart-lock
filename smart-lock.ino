...
#define NUMBER_OF_ALLOWED_CARDS 1
...
void log(entryEvent event){
  ...
  Serial.print(",");
  Serial.print((EEPROM[1] << 8) + EEPROM[0] + 5);
  Serial.print(",");
  Serial.println(addr);
...
void checkForDisplayBacklight(){
  analogWrite(DISPLAY_BACKLIGHT_PIN, constrain(map(displayOffTime - millis() + 5000 / 2, 0, 5000 / 2, 0, 255), 0, 255));
}
...
void pinCodeEntry(int num){
  ...
  if(pinTrial[0] == pinTrial[1] && pinTrial[1] == pinTrial[2] && (pinTrial[0] + pinTrial[1] + pinTrial[2]) == 0x12){
